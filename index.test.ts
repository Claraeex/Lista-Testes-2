...
function maioresDeIdade(vetor: Pessoa[]): Pessoa[] {
    ...
    let maiorIdade = vetor.filter(pessoa => pessoa.idade >= 18);
    return maiorIdade
}
...
//Os testes são usados para basicamente verificar se a função que você está fazendo está correta

//Aqui está a função criada para realizar o que foi pedido no teste
...
//Início do seu código
//O teste pede para unir os três caracteres que foram dados. A expectativa é que ele retorne "Hello World! :D"

test('Deve retornar "Hello World! :D"', () => {
    expect(helloWorld()).toEqual('Hello World! :D')
})

test('Deve retornar "Hello World! :D"', () => {
    let teste = helloWorld();
    expect(teste).not.toHaveLength(0); 
}) 
